...
from custom_optimizers import signSGD, FrankWolfe, ProjectionGD, MirrorGD
...
def make_optimizer(
    ...
    **optimizer_kwargs,
    ...
    if type == 'schedule_free':
        base_optimizer = torch.optim.AdamW(parameter_groups, lr=1e-5, weight_decay = 0 )
        optimizer = ScheduleFreeWrapper( base_optimizer, momentum=0.9, weight_decay_at_y=0.1)
        optimizer.train()
        return optimizer
    elif type == 'mechanize' :
        optimizer = mechanize(torch.optim.AdamW, s_decay=0.0, betas=(0.999,0.999999), store_delta=False)(parameter_groups, lr=1e-5, weight_decay = 0)
        return optimizer
    elif type == 'my_adamw':
        adamw_optimizer = torch.optim.AdamW(parameter_groups, lr=1e-5, weight_decay = 1e-6 )
        return adamw_optimizer
    
    elif type == 'my_sgd':
        sgd_optimizer = optim.SGD(parameter_groups, lr=1e-2, momentum=0.9)
        return sgd_optimizer
    elif type == 'my_lion':
        lion_optimizer = Lion(parameter_groups, lr=1e-4, weight_decay=1e-5)
        return lion_optimizer
    elif type == 'my_prodigy':
        prodigy_optimizer = Prodigy(parameter_groups, lr=1e-2, weight_decay=1e-3)
        return prodigy_optimizer
    elif type == 'my_rmsprop':
        rmsprob_optimizer = optim.RMSprop(parameter_groups, lr=1e-5, alpha=0.99)
        return rmsprob_optimizer
    elif type == 'my_signsgd':
        signsgd_optimizer = signSGD(parameter_groups, lr=1e-5)
        return signsgd_optimizer
    elif type == 'my_adagrad':
        adagrad_optimizer = optim.Adagrad(parameter_groups, lr=1e-2)
        return adagrad_optimizer
    elif type == 'my_lbfgs':
        # Does not support parameter_groups
        lbfgs_optimizer = torch.optim.LBFGS(module.parameters(), lr=1e-5 , history_size= 40, line_search_fn=True)
        return lbfgs_optimizer 
    elif type =='my_FW_Vanilla_R_2':
        lmo = lambda x, R: - R * x / x.norm(p=2)
        frankwolfe_optimizer = FrankWolfe(parameter_groups , lmo =lmo, direction='vanila_lr', scale = 1.1, lr = 1e-1)
        return frankwolfe_optimizer
    elif type == 'my_projgrad_R_2':
        proj = lambda x, R: R * x / x.norm(p=2)
        projGD_optimizer = ProjectionGD(parameter_groups , proj = proj, scale = 10, lr = 1e-1)
        return projGD_optimizer
    elif type == 'my_mirrorGD_entropy':
        def mirror_entropy_simplex( x, g, R ):
            value = torch.exp(-g) * x 
            return R * value / value.sum()
        mirrorgd_optimizer = MirrorGD(params=parameter_groups,mirror=mirror_entropy_simplex, lr = 1e-3, scale = 1)
        return mirrorgd_optimizer
